...
class Move(models.Model):
    ...
    def write(self, vals):
        ...
        #_logger.info('PASO POR AQUI grabando datos de factura X2B ' + str(vals))
        if self:
            for rec in self:
                _logger.info('PASO POR AQUI grabando datos de factura X2B t ' + str(vals))
                _logger.info('PASO POR AQUI grabando datos de factura X2B s ' + str(rec))
                # Obtener el contexto actual
                context = self.env.context

                # Imprimir las variables de contexto en el registro
                _logger.info("Variables de contexto: %s", context)
                if rec.company_id.sh_restrict_for_close_period and rec.period_id.state in ['done','reopen']:
                    _logger.info('PASO POR AQUI X2')
                    if 'needed_terms_dirty' in vals:
                        valor = vals['needed_terms_dirty']
                        if not valor:
                            return rslt
                    if 'is_paid_detraction' in vals:
                        valor = vals['is_paid_detraction']
                        if valor:
                            return rslt
                    if not vals:
                        return rslt
                    if self.ref:
                        _logger.info('PASO POR AQUI X2A')
                        raise UserError(_("You can not Select Date from Closed Fiscal Period / Closed Fiscal Year. >> " + str(self.name) + " >> " + str(self.ref)))
                    else:
                        _logger.info('PASO POR AQUI X2B val ' + str(vals))
                        _logger.info('PASO POR AQUI X2B  rec ' + str(rec))
                        _logger.info('PASO POR AQUI X2B2 ' + str(rslt))
                        
                        
                        raise UserError(_("You can not Select Date from Closed Fiscal Period / Closed Fiscal Year. >> " + str(self.name)))
        return rslt
