...
#
            ...
            try:
                ...
                self._cr.execute('''
                    INSERT INTO account_move_line (
                         name, display_type,create_uid, create_date, write_uid, write_date,
                      ...
                      (SELECT name, %s,create_uid, create_date, write_uid, write_date,
                       ...
                       WHERE account_id IN %s AND reconciled IS False 
                         AND ''' + query_line + ''')''', ('product',new_journal.id, period.id, period.date_start, move_id.id, tuple(account_ids),))
        ...
        self._cr.execute('''
            ...
            WHERE a.company_id = %s
              AND a.internal_group IN ('asset','liability')
              AND a.reconcile = %s''', (company_id, False))
        account_ids = map(lambda x: x[0], self._cr.fetchall())

        query_1st_part = """
                INSERT INTO account_move_line (
                     display_type,debit, credit, name, date, move_id, journal_id, period_id,
        ...
        for account in account_ids:
            ...
            if account_balance != 0.0:
                ...
                query_2nd_part_args += ('product',account_balance > 0 and account_balance or 0.0,
        ...
        # Credit Line
        query_credit_part = "(%s,%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)"
        account = new_journal.payment_debit_account_id
        query_credit_part_args = ('product',total_credit,
                                  0.0,
        ...
        query_credit_part_args = ('product',0.0,
                                  total_debit,
